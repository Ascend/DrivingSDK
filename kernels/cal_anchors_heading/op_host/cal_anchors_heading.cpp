...
static ge::graphStatus TilingForCalAnchorsHeading(gert::TilingContext *context) {
    ...
    if (dTypePtr == nullptr || anchorsShapePtr == nullptr || originPosShapePtr == nullptr ||
        headingShapePtr == nullptr) {
        return ge::GRAPH_FAILED;
    }
    ...
    uint32_t copyInDataBlockElemCountAligned =
        (Ceil(taskLength * ELEM_BYTE_SIZE, DATA_BLOCK_SIZE) + copyInLocalStride) * (DATA_BLOCK_SIZE / ELEM_BYTE_SIZE);

    uint32_t singleLoopTask = ubSize / (taskMemAlignedByte * UB_TASK_BUFFER_COUNT);
    singleLoopTask = std::min(
        std::min(singleLoopTask, (MAX_REPEAT_TIMES * ONE_REPEAT_BYTE_SIZE) / taskMemAlignedByte), MAX_REPEAT_TIMES);
