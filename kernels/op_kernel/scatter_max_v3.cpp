...
extern "C" __global__ __aicore__ void scatter_max_v3(
    GM_ADDR src, GM_ADDR idx, GM_ADDR res, GM_ADDR argmax, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(TILING_KEY_SMALL_TAIL)) {
        KernelScatterMaxV3<true> op(src, idx, res, argmax, &tiling_data, &pipe);
        op.Process();
    } else {
        KernelScatterMaxV3<false> op(src, idx, res, argmax, &tiling_data, &pipe);
        op.Process();
    }
}
