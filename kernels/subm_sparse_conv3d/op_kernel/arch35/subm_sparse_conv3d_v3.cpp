...
template <typename T> class KernelSubmSparseConv3dV3 {
  public:
    ...
    __aicore__ inline void InitTiling(SubmSparseConv3dV3TilingData *tilingData) {
        ...
        if ASCEND_IS_AIC {
            ...
            if (blkIdx_ < aicBigCoreCount) {
                ...
                taskStartOffset_ = (aicTaskCount + 1) * aicBigCoreCount + aicTaskCount * (blkIdx_ - aicBigCoreCount);
    ...
    __aicore__ inline void InitGM(GM_ADDR feature, GM_ADDR weight, GM_ADDR indices, GM_ADDR indices_offset,
        ...
        matmulResultGM1_.SetGlobalBuffer(
            (__gm__ float *)(workspace + 2 * totalTaskCount_ * inChannels_ * byteSizePerElements_));
    ...
    __aicore__ inline void InitUB() {
        if ASCEND_IS_AIV {
            ...
            scatterFeatureLocal1_ =
                validIndicesForGatherFeatureLocal2_[singleLoopTaskAligned_].template ReinterpretCast<float>();
            scatterFeatureLocal2_ = scatterFeatureLocal1_[2 * scatterBufLen_ * outChannelsAligned_];

            gatherFeatureLocal1_ =
                validIndicesForGatherFeatureLocal2_[singleLoopTaskAligned_].template ReinterpretCast<T>();
    ...
    __aicore__ inline void GatherFeature(int32_t taskOffset, int32_t vfTaskCount) {
        ...
        for (int32_t i = 0; i < vfTaskCount; i += gatherBufLen_) {
            ...
            if (inChannels_ % (BYTE_SIZE_PER_BLOCK / byteSizePerElements_) == 0) {
                ...
                uint32_t threadNum =
                    min(inChannels_ * processTaskCountCurLoop / elementsCountPerVectorDtype, THREAD_NUM);
                ...
                __gm__ volatile FEATURE_DTYPE *inputFeatureGlobal =
                    (__gm__ volatile FEATURE_DTYPE *)inputFeatureGM_.GetPhyAddr();
                __ubuf__ FEATURE_DTYPE *gatheredFeature1Local =
                    (__ubuf__ FEATURE_DTYPE *)gatherFeatureLocal1_[ping * gatherBufLen_ * inChannelsAligned_]
                        .GetPhyAddr();
                __ubuf__ FEATURE_DTYPE *gatheredFeature2Local =
                    (__ubuf__ FEATURE_DTYPE *)gatherFeatureLocal2_[ping * gatherBufLen_ * inChannelsAligned_]
                        .GetPhyAddr();
                Simt::VF_CALL<GatherFeatureVectorDTypeSimt<T, FEATURE_DTYPE>>(Simt::Dim3{xThreadNum, yThreadNum},
                    validIndices1Local + i, validIndices2Local + i, inputFeatureGlobal, gatheredFeature1Local,
                    gatheredFeature2Local, vectorDtypeCountPerChannel, processTaskCountCurLoop, taskOffset);
            } else {
                ...
                __ubuf__ T *gatheredFeature1Local =
                    (__ubuf__ T *)gatherFeatureLocal1_[ping * gatherBufLen_ * inChannelsAligned_].GetPhyAddr();
                __ubuf__ T *gatheredFeature2Local =
                    (__ubuf__ T *)gatherFeatureLocal2_[ping * gatherBufLen_ * inChannelsAligned_].GetPhyAddr();
                Simt::VF_CALL<GatherFeatureSimt<T>>(Simt::Dim3{threadNum}, validIndices1Local + i,
                    validIndices2Local + i, inputFeatureGlobal, gatheredFeature1Local, gatheredFeature2Local,
                    inChannels_, processTaskCountCurLoop, taskOffset);
            }
            SetFlag<HardEvent::V_MTE3>(0);
            WaitFlag<HardEvent::V_MTE3>(0);

            DataCopyPad(mmFeatureGM1_[(taskOffset + i) * inChannels_],
                gatherFeatureLocal1_[ping * gatherBufLen_ * inChannelsAligned_],
                {static_cast<uint16_t>(processTaskCountCurLoop),
                    static_cast<uint32_t>(inChannels_ * byteSizePerElements_), 0, 0, 0});
            DataCopyPad(mmFeatureGM2_[(taskOffset + i) * inChannels_],
                gatherFeatureLocal2_[ping * gatherBufLen_ * inChannelsAligned_],
                {static_cast<uint16_t>(processTaskCountCurLoop),
                    static_cast<uint32_t>(inChannels_ * byteSizePerElements_), 0, 0, 0});
    ...
    __aicore__ inline void ScatterFeature(const int32_t &taskOffset, const int32_t &vfTaskCount) {
        ...
        for (int32_t i = 0; i < vfTaskCount; i += scatterBufLen_) {
            ...
            __ubuf__ float *matmulRes1Local =
                (__ubuf__ float *)scatterFeatureLocal1_[ping * scatterBufLen_ * outChannelsAligned_].GetPhyAddr();
            __ubuf__ float *matmulRes2Local =
                (__ubuf__ float *)scatterFeatureLocal2_[ping * scatterBufLen_ * outChannelsAligned_].GetPhyAddr();

            WaitFlag<HardEvent::V_MTE2>(ping);
            DataCopyPad(scatterFeatureLocal1_[ping * scatterBufLen_ * outChannelsAligned_],
                matmulResultGM1_[(taskOffset + i) * outChannels_],
                {static_cast<uint16_t>(processTaskCountCurLoop), static_cast<uint32_t>(outChannels_ * FP32_BYTE_SIZE),
                    0, 0, 0},
                {false, 0, 0, 0});
            DataCopyPad(scatterFeatureLocal2_[ping * scatterBufLen_ * outChannelsAligned_],
                matmulResultGM2_[(taskOffset + i) * outChannels_],
                {static_cast<uint16_t>(processTaskCountCurLoop), static_cast<uint32_t>(outChannels_ * FP32_BYTE_SIZE),
                    0, 0, 0},
                {false, 0, 0, 0});
            ...
            Simt::VF_CALL<ScatterFeatureSimt<T>>(Simt::Dim3{xThreadCount, yThreadCount}, validIndices1Local + i,
                validIndices2Local + i, outputFeatureGlobal, matmulRes1Local, matmulRes2Local, outChannels_,
                processTaskCountCurLoop, taskOffset);
        ...
        if ASCEND_IS_AIV {
            if (useTwolevelMap_) {
                Simt::VF_CALL<CopyInIndicesTwoMapNoKeySimt>(Simt::Dim3{THREAD_NUM, 1, 1},
                    (__gm__ volatile int32_t *)indicesOffsetGM_.GetPhyAddr(),
                    (__gm__ volatile int4 *)indicesGM_.GetPhyAddr(), (__gm__ volatile int32_t *)map1GM_.GetPhyAddr(),
                    (__gm__ volatile int32_t *)map2GM_.GetPhyAddr(), blkIdx_, aivNum_, totalTaskCount_, spatialShape0_,
            ...
            DataCacheCleanAndInvalid<int32_t, CacheLine::ENTIRE_DATA_CACHE, AscendC::DcciDst::CACHELINE_OUT>(
                indicesOffsetGM_);
    ...
    __aicore__ inline void ComputeValidPointCount(int32_t k, int32_t &validTaskCount, int32_t taskCount) {
        ...
        CompareScalar(validMaskLocal, validIndicesForGatherFeatureLocal1_, static_cast<int32_t>(-1), CMPMODE::NE,
            taskCountAligned);
    ...
    __aicore__ inline void ProcessSparseMatmul() {
        if ASCEND_IS_AIV {
            ...
            for (int32_t k = 0; k < kernelSize_ / 2; k++) {
                ...
                for (int32_t taskOffset = aivTaskStartOffset; taskOffset < totalTaskCount_;
                     taskOffset += singleLoopTask_ * aivNum_) {
        ...
        if ASCEND_IS_AIC {
            ...
            for (int32_t k = 0; k < kernelSize_ / 2; k++) {
                int8_t ping = 0;
                for (int32_t aicTaskOffset = aicTaskStartOffset; aicTaskOffset < totalTaskCount_;
                     aicTaskOffset += (2 * singleLoopTask_) * aicNum_) {
                    CrossCoreWaitFlag<0x2>(ping);
                    CrossCoreWaitFlag<0x2>(ping);
                    DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE, DcciDst::CACHELINE_OUT>(
                        validTaskCountGM_[aicTaskOffset + k * totalTaskCountAligned_]);
                    DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE, DcciDst::CACHELINE_OUT>(
                        validTaskCountGM_[aicTaskOffset + k * totalTaskCountAligned_ + singleLoopTask_]);
                    int32_t validTaskCount1 = validTaskCountGM_.GetValue(k * totalTaskCountAligned_ + aicTaskOffset);
                    int32_t validTaskCount2 =
                        validTaskCountGM_.GetValue(k * totalTaskCountAligned_ + aicTaskOffset + singleLoopTask_);
  ...
  private:
    ...
    int32_t k0_, k1_, k2_, kernelSize_, batchSize_, inChannels_, outChannels_, spatialShape0_, spatialShape1_,
        byteSizePerElements_, totalTaskCount_, totalTaskCountAligned_, stage2SingleLoopTask_, withKey_, spatialShape2_,
        spatialShape0_times_1_, spatialShape1_times_2_, coreTaskCount_, stage2SingleLoopTaskAligned_, singleLoopTask_,
        inChannelsAligned_, outChannelsAligned_, kernelSizeAligned_, taskStartOffset_, singleLoopTaskAligned_,
        k1Aligned_, k2Aligned_, mapValBufSize_, gatherBufLen_, scatterBufLen_, availableUBSize_, curLoopValidTask_,
        preLoopValidTask_, matmulCount0_ = 0, matmulCount1_ = 0;
    ...
    LocalTensor<int32_t> inputIndicesLocal_, indicesOffsetLocal_, batchIdxLocal_, spatial0Local_,
        validIndicesForGatherFeatureLocal1_, validIndicesForGatherFeatureLocal2_, reOrderedIndicesLocal_,
        spatial1Local_, spatial2Local_, mapValLocal_, map1ValLocal_, maskForGatherLocal_,
        validIndicesForScatterFeatureLocal_, maskForScatterLocal_;
...
extern "C" __global__ __aicore__ void subm_sparse_conv3d_v3(GM_ADDR feature, GM_ADDR weight, GM_ADDR indices,
    ...
    op.Init(&pipe, feature, weight, indices, indices_offset, map1, map2, feature_out, out_indices_offset, &tilingData,
        usrWorkspace);
    op.Process();
}
