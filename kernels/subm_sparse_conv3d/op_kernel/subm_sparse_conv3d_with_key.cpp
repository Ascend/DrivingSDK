...
class KernelSubmSparseConv3dWithKey {
public:
    __aicore__ inline KernelSubmSparseConv3dWithKey() {}

    TQue<QuePosition::VECIN, 1> inQueueOffset, inQueueValid, inQueueFeature;
    GlobalTensor<DTYPE_OUTIDX_OFFSET> outidxOffsetGm;
    GlobalTensor<DTYPE_OUTIDX_OFFSET> validIndicesGm;
    GlobalTensor<DTYPE_GRAD_OUT_FEATURES> FeaturesGm;
    ...
    LocalTensor<DTYPE_GRAD_OUT_FEATURES> feature_ub;
    ...
    __aicore__ inline void Init(GM_ADDR ouidx_offset, GM_ADDR valid_indices,
                                ...
                                GM_ADDR output,
                                GM_ADDR workspace,
                                SubmSparseConv3dWithKeyTilingData *tiling_data, TPipe* pipe)
    {
        ...
        FeaturesGm.SetGlobalBuffer((__gm__ DTYPE_GRAD_OUT_FEATURES*)features, this->indices_number * this->inchannel);
        outputGm.SetGlobalBuffer(
            (__gm__ DTYPE_GRAD_OUT_FEATURES*)output, this->indices_number * total_kernel_size * this->inchannel);
        ...
        pipe->InitBuffer(inQueueFeature, 1, this->inchannel * total_kernel_size * sizeof(DTYPE_GRAD_OUT_FEATURES));
        copyParams_offset = {1, (uint16_t)(this->available_ub_size * sizeof(DTYPE_OUTIDX_OFFSET)), 0, 0};
        copyParams_grad = {1, (uint16_t)(this->inchannel * sizeof(DTYPE_GRAD_OUT_FEATURES)), 0, 0};
    }
...
private:
    __aicore__ inline void IndicesComputeAlgin(int32_t progress, int32_t tensor_size, uint64_t address)
    {
        ...
        feature_ub = inQueueFeature.AllocTensor<DTYPE_GRAD_OUT_FEATURES>();
        // 计算indices的loop参数
        copyParams_valid = {1, (uint16_t)(tensor_size * sizeof(DTYPE_OUTIDX_OFFSET)), 0, 0};
        auto inchannel_ailgn_32b = AlignUp(this->inchannel, 8);
        DataCopyPadParams gradpadParams = {true, 0, (uint8_t)(inchannel_ailgn_32b-this->inchannel), 0};
        DataCopyParams copyParams_out = {(uint16_t)(total_kernel_size),
                                         (uint16_t)(this->inchannel * sizeof(DTYPE_OUTIDX_OFFSET)), 0, 0};
        ...
        for (int32_t i = 0; i < tensor_size/64; i++) {
            for (int32_t j = 0; j < 64; j++) {
                ...
                DataCopyPad(feature_ub[j * inchannel_ailgn_32b], FeaturesGm[offset_idx * this->inchannel],
            ...
            for (int32_t j = 0; j< 64; j++) {
                ...
                DataCopyPad(outputGm[(int32_t)(feature_idx * total_kernel_size * this->inchannel + valid_idx* this->inchannel)],
                            feature_ub[j * inchannel_ailgn_32b], copyParams_grad);
            }
            PipeBarrier<PIPE_ALL>();
        }
    ...
    __aicore__ inline void IndicesCompute(int32_t progress, int32_t tensor_size, uint64_t address)
    {
        ...
        for (int32_t i = 0; i < tensor_size; i++) {
            ...
            DataCopyPad(feature_ub, FeaturesGm[offset_idx * this->inchannel], copyParams_grad, padParams);
            set_flag(PIPE_MTE2, PIPE_MTE3, EVENT_ID0);
            wait_flag(PIPE_MTE2, PIPE_MTE3, EVENT_ID0);
            DataCopyPad(outputGm[(int32_t)(feature_idx * total_kernel_size * this->inchannel + valid_idx* this->inchannel)],
                        feature_ub, copyParams_grad);
            PipeBarrier<PIPE_ALL>();
        }
        inQueueOffset.FreeTensor(offset_ub);
        inQueueValid.FreeTensor(indices_ub);
...
extern "C" __global__ __aicore__ void subm_sparse_conv3d_with_key(GM_ADDR ouidx_offset, GM_ADDR valid_indices,
                                                        GM_ADDR grad_out_features,
                                                        GM_ADDR grad_out_features_iml2col,
                                                        GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tiling_data, tiling);
    KernelSubmSparseConv3dWithKey op;
    TPipe pipe;
