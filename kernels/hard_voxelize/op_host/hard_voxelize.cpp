...
ge::graphStatus TaskScheduleForCopy(gert::TilingContext* context, optiling::HardVoxelizeTilingData& tilingData)
{
    ...
    int32_t usedUB = RESERVE_UB + 2 * AlignUp(featNum, ONE_BLK_SIZE) * sizeof(float) + 3 * ONE_BLK_SIZE + FREE_NUM * sizeof(int32_t) +
        maxPoints * featNumAlign * B32_BYTE_SIZE + maxPointsAlign * B32_BYTE_SIZE;
