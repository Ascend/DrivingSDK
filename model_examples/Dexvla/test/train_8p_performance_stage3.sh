...
#获取性能数据，不需要修改
#吞吐量
ActualFPS=$(grep -o "train_samples_per_second': [0-9.]*" ${test_path_dir}/output/train_stage3_performance_8p_base_fp32.log | awk 'END {print $NF}')

#打印，不需要修改
echo "Final Performance images/sec : $ActualFPS"

#loss值，不需要修改
ActualLoss=$(grep -o "train_loss': [0-9.]*" ${test_path_dir}/output/train_stage3_performance_8p_base_fp32.log | awk 'END {print $NF}')
