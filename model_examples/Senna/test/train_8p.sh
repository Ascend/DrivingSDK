...
sh train_tools/train_senna_llava.sh \
    >$cur_path/test/output2/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
...
# 输出训练精度IoU,需要模型审视修改
accuracy=` grep -a 'Planning Accuracy:' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Planning Accuracy: " '{print $2}' | awk '{last=$1} END {print last}'`
...
echo "Accuracy = ${accuracy}" >>${test_path_dir}/output/$ASCEND_DEVICE_ID/${CaseName}.log
