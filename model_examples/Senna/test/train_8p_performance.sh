...
sh train_tools/pretrain_senna_llava.sh \
    >$cur_path/test/output1/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
train_steps_per_second=`grep -a '{'train_runtime''  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F "train_steps_per_second': " '{print $2}' | awk -F ","`

Iteration_time=$train_steps_per_second
FPS=$(awk BEGIN'{print ('$BatchSize' * '$WORLD_SIZE') * '$Iteration_time'}')
# 打印，不需要修改
echo "train_steps_per_second : $Iteration_time"
echo "FPS : $FPS"
...
echo "train_steps_per_second = ${Iteration_time}" >>${test_path_dir}/output/$ASCEND_DEVICE_ID/${CaseName}_perf_report.log
