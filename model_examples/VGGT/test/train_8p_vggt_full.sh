...
# 训练用例名称
CASE_NAME=${NETWORK}_${WORLD_SIZE}p_full
...
#修改参数
sed -i 's/limit_train_batches: 400/limit_train_batches: 800/' config/default.yaml
sed -i 's/max_epochs: 1/max_epochs: 20/' config/default.yaml
...
train_loss_reg_depth=$(grep -a 'Train Epoch: ' ${OUTPUT_PATH}/train.log | tail -n 1 | awk -F "Loss/train_loss_reg_depth: " '{print 2}'| awk -F '[()]' '{print $2}')
echo "[VGGT] Loss : ${train_loss_reg_depth}"
...
echo "Loss = ${train_loss_reg_depth}" >> ${OUTPUT_PATH}/${CASE_NAME}.log