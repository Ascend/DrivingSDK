...
bash ./tools/dist_train.sh ${py_config} ${world_size} > ${output_path_dir}/train_8p_full.log 2>&1 &
wait
...
#获取性能数据，不需要修改
#单迭代训练时长，不需要修改
TrainingTime=$(grep -o ", time: [0-9.]" ${test_path_dir}/output/train_8p_full.log | tail -n +15 | grep -o "[0-9.]" | awk '{sum += $1} END {print sum/NR}')
...
#loss值，不需要修改
ActualLoss=$(grep -o "loss: [0-9.]*" ${test_path_dir}/output/train_8p_full.log | awk 'END {print $NF}')

#NDS值
NDS=$(grep -o "pts_bbox_NuScenes/NDS: [0-9.]*" ${test_path_dir}/output/train_8p_full.log | awk 'END {print $NF}')

#mAP值
mAP=$(grep -o "pts_bbox_NuScenes/mAP: [0-9.]*" ${test_path_dir}/output/train_8p_full.log | awk 'END {print $NF}')
