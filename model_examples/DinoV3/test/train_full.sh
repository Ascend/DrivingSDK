...
#训练开始时间，不需要修改
start_time=$(date +%s)
torchrun --nproc_per_node=$world_size  --nnodes=1  --node-rank=0 --master_addr=127.0.0.1  --master_port=29501  dinov3/train/train.py --config-file dinov3/configs/train/vitl_im1k_lin834.yaml \
...
#获取性能数据，不需要修改
#吞吐量
avg_time=$(grep ")  time:" ${test_path_dir}/output/train_full_8p_base_fp32.log | tail -5 | awk -F'time: ' '{print $2}' | awk '{print $1}' | awk '{sum+=$1} END {print sum/NR}')
ActualFPS=$(awk BEGIN'{print ('$world_size' * '$batch_size') / '$avg_time'}')
#打印，不需要修改
echo "Final Performance images/sec : $ActualFPS"

#loss值，不需要修改
ActualLoss=$(grep -o "total_loss: [0-9.]*" ${test_path_dir}/output/train_full_8p_base_fp32.log | awk 'END {print $NF}')
