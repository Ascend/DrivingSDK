...
#修改参数
sed -i "s/batch_size: [0-9]\+/batch_size: $BATCH_SIZE/g" config/nwm_cdit_xl.yaml
...
train_loss=$(grep -a 'Train Loss: ' "${OUTPUT_PATH}/train.log" | tail -n 1 | awk -F 'Train Loss: ' '{print $2}' | awk -F ', ' '{print $1}')
echo "[${NETWORK}] Loss : ${train_loss}"
...
echo "Loss = ${train_loss}" >> ${OUTPUT_PATH}/${CASE_NAME}.log