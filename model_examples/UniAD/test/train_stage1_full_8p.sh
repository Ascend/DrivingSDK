...
if [ -d ${cur_path}/test/output/full/stage1 ]; then
  rm -rf ${cur_path}/test/output/full/stage1
  mkdir -p ${cur_path}/test/output/full/stage1
else
  mkdir -p ${cur_path}/test/output/full/stage1
fi
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
avg_time=`grep -a 'mmdet - INFO - Epoch '  ${test_path_dir}/output/full/stage1/train_full.log|awk -F "time: " '{print $2}' | awk -F ", " '{print $1}' | awk 'NR>10 {sum+=$1; count++} END {if (count != 0) printf("%.3f",sum/count)}'`
...
# 训练总时长
TrainingTime=`grep -a 'Time'  ${test_path_dir}/output/full/stage1/train_full.log|awk -F "Time: " '{print $2}'|awk -F "," '{print $1}'| awk '{a+=$1} END {printf("%.3f",a)}'`

# 关键信息打印到${CaseName}.log中，不需要修改
echo "Network = ${Network}" >${test_path_dir}/output/full/stage1/${CaseName}.log
echo "RankSize = ${WORLD_SIZE}" >>${test_path_dir}/output/full/stage1/${CaseName}.log
echo "BatchSize = ${BatchSize}" >>${test_path_dir}/output/full/stage1/${CaseName}.log
echo "DeviceType = ${DeviceType}" >>${test_path_dir}/output/full/stage1/${CaseName}.log
