...
#训练开始时间，不需要修改
start_time=$(date +%s)
bash torch_run.sh ${epoch} > ${test_path_dir}/output/train_full_8p_base_fp32.log 2>&1 &
...
#吞吐量
ActualFPS=$(awk BEGIN'{print ('$batch_size') * '$TrainingTime'}')

#打印，不需要修改
echo "Final Performance images/sec : $ActualFPS"

#loss值，不需要修改
ActualLoss=$(grep -o "epoch train loss: [0-9.]*" ${test_path_dir}/output/train_full_8p_base_fp32.log | awk 'END {print $NF}')
