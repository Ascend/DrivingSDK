...
from utils_pt import quintic_spline, fitting_traj_by_qs
...
class highwayTrajDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for id, target in enumerate(centGrid):
            if target:
                ...
                grid = self.getGrid(dsId, target, t)
                front_id = grid[self.grid_size[0] * (self.grid_size[1] // 2 - 1)]  # 中上格子

                if front_id:
                    # 空间头距
                    s = self.getLocation(dsId, front_id, t)[1] - self.getLocation(dsId, target, t)[1]
                    dv = self.getDynamic(dsId, target, t)[0] - self.getDynamic(dsId, front_id, t)[0]
                    v_front = self.getDynamic(dsId, front_id, t)[0]
                else:
                    s = 0.0
                    dv = 0.0
                    v_front = 0.0

                space_headways.append(s)
                relative_velocities.append(dv)
                preceding_velocities.append(v_front)
        return planFuts, nbsHists, \
               targsHists, targsFuts, targsLonEnc, targsLatEnc, \
               centGrid, planGridLocs, idx, space_headways, relative_velocities, preceding_velocities
    ...
    def itsAllAroundSizes(self, idx):
        ...
        for _, target in enumerate(centGrid):
            if target:
    ...
    def collate_fn(self, samples):
        targs_batch_size = 0
        nbs_batch_size = 0
        for _, nbsHists, targsHists, _, _, _, _, _, _, _, _, _ in samples:
        ...
        space_h_batch = torch.zeros(self.fut_len, targs_batch_size)
        dv_batch      = torch.zeros(self.fut_len, targs_batch_size)
        v_pre_batch   = torch.zeros(self.fut_len, targs_batch_size)
        targetCount = 0
        nbCount = 0
        for i, (planFuts, nbsHists, targsHists, targsFuts, targsLonEnc, targsLatEnc,
            centGrid, planGridLocs, idx, space_headways, relative_velocities, preceding_velocities) in enumerate(samples):
            ...
            for j in range(len(targsFuts)):
                ...
                space_h_batch[:, targetCount] = torch.tensor(space_headways[j]).repeat(self.fut_len)
                dv_batch[:, targetCount]      = torch.tensor(relative_velocities[j]).repeat(self.fut_len)
                v_pre_batch[:, targetCount]   = torch.tensor(preceding_velocities[j]).repeat(self.fut_len)
        ...
        return nbsHist_batch, nbsMask_batch, \
            ...
            space_h_batch, dv_batch, v_pre_batch
