...
# @ATTENTION.register_module()
# class TPVSelfAttention(BaseModule):
...
#             embed_dims, num_heads * num_points)
...
#             query = query.permute(1, 0, 2)  # change to (bs, num_query ,embed_dims)
#         bs, num_query, _ = query.shape
#         assert (spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum() == num_query

#         value = self.value_proj(query)
#         value = value.reshape(bs, num_query, self.num_heads, -1)
