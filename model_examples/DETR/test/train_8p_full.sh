...
nohup torchrun --standalone --nnodes=1 \
    ...
    --epochs=${epochs} \
    --coco_path=${data_path} \
    --output_dir=${output_path_dir} > ${output_path_dir}/train_8p_full.log 2>&1 &
...
# 输出性能
avg_time=`grep -P 'Epoch: \[\d+\] Total time' ${output_path_dir}/train_8p_full.log | tail -n 5 | grep -oP '(?<=\()[0-9]+\.[0-9]+' | awk '{sum+=$1; count++} END {if(count>0) print sum/count}'`
