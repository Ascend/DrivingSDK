...
sed -i "256s/interval=1/interval=50/g" projects/configs/bevformer/bevformer_base.py
...
#获取性能数据，不需要修改
#单迭代训练时长，不需要修改
TrainingTime=$(grep -o ", time: [0-9.]*" ${test_path_dir}/output/train_performance_8p_base_fp32.log | tail -n 8 | grep -o "[0-9.]*" | awk '{sum += $1} END {print sum/NR}')
