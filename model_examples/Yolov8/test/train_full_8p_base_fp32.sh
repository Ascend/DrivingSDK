...
# 计算FPS的平均值
total_FPS=`grep -oP 'FPS:\K\s*(\d+\.?\d*)' ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_full_8p.log | tail -n 78 | awk '{sum += $1} END {print sum}'`
averageFPS=$(echo "scale=2; $total_FPS/78" | bc)

# 取mAP50的值
mAP50=$(grep -w 'all' ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_full_8p.log | awk -F "all" '{print $2}' | awk -F "    " '{print $7}' |  tail -n1)
# 取mAP50-95的值
mAP50_95=$(grep -w 'all' ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_full_8p.log | awk -F "all" '{print $2}' | awk -F "    " '{print $8}' |  tail -n1)
