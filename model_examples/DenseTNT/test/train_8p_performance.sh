...
nohup python src/run.py --argoverse --argoverse2 --future_frame_num 60 \
  --do_train --data_dir ${data_path} --output_dir ${output_path} --num_train_epochs 0 \
...
nohup python src/run.py --argoverse --argoverse2 --future_frame_num 60 \
  ...
  --other_params \
    ...
    goals_2D enhance_global_graph subdivide goal_scoring laneGCN point_sub_graph \
    lane_scoring complete_traj complete_traj-3 \
> ${output_path_dir}/train_8p_performance.log 2>&1 &
...
# 从 log 中获取性能
avg_time=`grep "loss=" ${output_path_dir}/train_8p_performance.log | tail -n 1 | grep -oP "[0-9]+\.[0-9]+it/s" | awk '{sum+=$1; count++} END {if(count>0) print sum/count}'`
# 从 log 中获取精度
FDE=`grep "FDE:" ${output_path_dir}/train_8p_performance.log | awk -F " " '{print $2}'| awk 'END {print}'`
MR=`grep "FDE:" ${output_path_dir}/train_8p_performance.log | awk -F ":" '{print $NF}'| awk 'END {print}'`
