...
#训练开始时间，不需要修改
start_time=$(date +%s)
python -m accelerate.commands.launch --num_processes=${world_size} --main_process_port=12345 lerobot/scripts/train.py      --dataset.repo_id=${dataset_dir}   --steps=1000  --log_freq=20  --save_freq=30000 --batch_size=${batch_size}     --policy.path=${policy_dir}> ${test_path_dir}/output/train_performance_8p_base_fp32.log 2>&1 &
...
#获取性能数据，不需要修改
#单迭代训练时长，不需要修改
epoch_start_time=`grep 'ts/train.py:' ${test_path_dir}/output/train_performance_8p_base_fp32.log | grep 'loss' | tail -10 | grep -o [0-9][0-9]:[0-9][0-9]:[0-9][0-9] | head -1`
epoch_end_time=`grep 'ts/train.py:' ${test_path_dir}/output/train_performance_8p_base_fp32.log | grep 'loss' | tail -10 | grep -o [0-9][0-9]:[0-9][0-9]:[0-9][0-9] | tail -1`
