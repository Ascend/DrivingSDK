...
constexpr int32_t X_IDX = 0;
constexpr int32_t Y_IDX = 1;
constexpr int32_t Z_IDX = 2;
constexpr int32_t XYZ_LOOP_COUNT = 3;
...
ge::graphStatus SetTilingDataFromAttr(const gert::TilingContext* context, optiling::PointToVoxelTilingData& tilingData)
{
    ...
    float coorYMin;
    float coorZMin;
    if (GetElementInListAttr(attrs, VOXEL_SIZES_IDX, X_IDX, voxelSizeX) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, VOXEL_SIZES_IDX, Y_IDX, voxelSizeY) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, VOXEL_SIZES_IDX, Z_IDX, voxelSizeZ) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, COOR_MINS_IDX, X_IDX, coorXMin) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, COOR_MINS_IDX, Y_IDX, coorYMin) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, COOR_MINS_IDX, Z_IDX, coorZMin) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, COOR_MINS_IDX, XYZ_LOOP_COUNT + X_IDX, coorXMax) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, COOR_MINS_IDX, XYZ_LOOP_COUNT + Y_IDX, coorYMax) != ge::GRAPH_SUCCESS ||
        GetElementInListAttr(attrs, COOR_MINS_IDX, XYZ_LOOP_COUNT + Z_IDX, coorZMax) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus TaskSchedule(gert::TilingContext* context, optiling::PointToVoxelTilingData& tilingData)
{
    ...
    avgPts = std::min(avgPts, AlignUp(DivCeil(totalPts, core_num), ONE_REPEAT_FLOAT_SIZE));
    if (avgPts == 0) {
        return ge::GRAPH_FAILED;
    }
