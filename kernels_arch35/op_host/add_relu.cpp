...
namespace optiling {
const uint64_t BLOCK_DIM = 8;
const uint64_t TILE_NUM = 8;
...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    uint64_t totalResult = context->GetInputTensor(0)->GetShapeSize();
    uint64_t coreData;
    uint64_t coreUsed;
    uint64_t coreLast;
    ...
    auto xDataTypePtr = context->GetInputDesc(0);
    auto xDataType = xDataTypePtr->GetDataType();
    int64_t byteSizePerElements = xDataType == ge::DT_FLOAT16? HALF_BYTE_SIZE : FLOAT_BYTE_SIZE;

    availableUbSize = (availableUbSize - 20 * 1024) / 3;
    availableUbSize = availableUbSize / 256 * 256 / byteSizePerElements;
    availableUbSize = availableUbSize > 15872 ? 15872 : availableUbSize;
