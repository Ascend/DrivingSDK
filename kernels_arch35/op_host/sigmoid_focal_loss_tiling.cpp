...
namespace optiling {
    static ge::graphStatus TilingForSigmoidFocalLoss(gert::TilingContext *context) {
...
namespace ge {
    static ge::graphStatus InferShapeForSigmoidFocalLoss(gert::InferShapeContext* context) {
        CHECK_NULLPTR(context);
        const gert::Shape* logit_shape = context->GetInputShape(0);
        gert::Shape* loss_shape = context->GetOutputShape(0);
        if (logit_shape == nullptr || logit_shape == nullptr) {
            return ge::GRAPH_FAILED;
        }
    ...
    static ge::graphStatus InferDataTypeForSigmoidFocalLoss(gert::InferDataTypeContext *context) {
        CHECK_NULLPTR(context);
...
namespace ops {
    class SigmoidFocalLoss: public OpDef {
        ...
        explicit SigmoidFocalLoss(const char* name) : OpDef(name) {
            ...
            this->SetInferShape(ge::InferShapeForSigmoidFocalLoss)
