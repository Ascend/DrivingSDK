...
__aicore__ inline void MultiScaleDeformableAttentionGrad<T>::init_buffer()
{
    ...
    pipe.InitBuffer(in_queue_sl, 1, ceil(num_heads * num_levels * num_point * DOUB,
                    t_per_block) * sizeof(T));
    pipe.InitBuffer(in_queue_aw, 1, ceil(num_heads * num_levels * num_point,
                    t_per_block) * sizeof(T));
                                                                                 ...
                                                                                 int32_t data_loc_w_ptr)
{
    ...
    pipe_barrier(PIPE_ALL);
    WholeReduceSum<T>(w3_v3_local, grad_h_weight_local, channels, num_point, 1, 1, (channels - 1) / t_per_block + 1);
    WholeReduceSum<T>(w4_v4_local, grad_w_weight_local, channels, num_point, 1, 1, (channels - 1) / t_per_block + 1);
    WholeReduceSum<T>(w2_v2_local, grad_weight_local, channels, num_point, 1, 1, (channels - 1) / t_per_block + 1);
    pipe_barrier(PIPE_ALL);
    SetAtomicAdd<T>();
    #ifndef __GET_CODE_CHANNEL__
    DataCopyParams copy_params{1, (uint16_t)(num_point * sizeof(float)), 0, 0};
    DataCopyPad(grad_attn_weight_gm[data_weight_ptr + cur_nl * num_point], w2_v2_local, copy_params);
    DataCopyPad(grad_sampling_loc_gm[data_loc_w_ptr + cur_nl * 2 * num_point], w4_v4_local, copy_params);
    DataCopyPad(grad_sampling_loc_gm[data_loc_w_ptr + cur_nl * 2 * num_point + num_point], w3_v3_local, copy_params);
    #endif
    SetAtomicNone();
    pipe_barrier(PIPE_ALL);
}
                                                                            ...
                                                                            int32_t data_loc_w_ptr)
{
    for (int32_t cur_nl = 0; cur_nl < num_levels; cur_nl++)
    {
        ...
        pipe_barrier(PIPE_ALL);
        post_process_levels(w, h, cur_nl, data_weight_ptr, data_loc_w_ptr);
    }
}
...
__aicore__ inline void MultiScaleDeformableAttentionGrad<T>::compute_mode_zero()
{
    ...
    for (int32_t b_nq_ind = start_task_id; b_nq_ind < start_task_id + cur_core_task_num; b_nq_ind++)
    {
        ...
        {
            pipe_barrier(PIPE_ALL);
            auto data_weight_ptr = (cur_b * num_query * num_heads + cur_q * num_heads +
                                    cur_nh) * num_levels * num_point;
                                                                                 ...
                                                                                 int32_t scalarValue, const int32_t calCount)
{
    ...
    for (int32_t loop_idx = 0; loop_idx < loop; loop_idx++)
    {
        Adds(dstLocal[loop_idx * max_repeat * mask], srcLocal[loop_idx * max_repeat * mask], scalarValue,
             mask, max_repeat, {DST_BLK_STRIDE, SRC_BLK_STRIDE, DST_REP_STRIDE, SRC_REP_STRIDE});
    }
    pipe_barrier(PIPE_ALL);
    tensor_offset = loop * max_repeat * mask;
    pipe_barrier(PIPE_ALL);
...
__aicore__ inline void MultiScaleDeformableAttentionGrad<T>::adds_template(const LocalTensor<T> &dstLocal,
    const LocalTensor<T> &srcLocal, T scalarValue, const int32_t calCount)
{
    int32_t unit = 256;
    int32_t max_repeat = 64;
    int32_t mask = unit / sizeof(T);
