...
public:
                                ...
                                DynamicScatterTilingData* tilingData)
    {
        ...
        if (GetBlockIdx() == 0) {
            InitOutput<T>(this->reducedFeatsGm, this->outLength, static_cast<T>(0.0));
        }
        SyncAll();
    }
...
private:
    ...
    __aicore__ inline void Compute(int32_t gmOffset, int32_t pointNum)
    {
        ...
        for (uint32_t idx = 0; idx < pointNum; idx++) {
            ...
            if (reduce_to > -1) {
                ...
                pipe_barrier(PIPE_ALL);
                this->CopyOutAdd(reducedFeatsGm, reduce_to * this->featsNum, featsLocal);
            }
        }
    ...
    __aicore__ inline void ComputeTail(int32_t pointNum)
    {
        ...
        for (uint32_t idx = 0; idx < pointNum; idx++) {
            int32_t reduce_to = coorsMapLocal.GetValue(idx);
            int32_t featsIndex = this->inputNum - idx - 1;
            if (reduce_to > -1) {
