    ...
    __aicore__ inline void Compute(uint32_t query)
    {
        ...
        for (uint32_t batch = 0; batch < batchSize; batch++) {
            ...
            for (uint32_t head = 0; head < numHeads; head++) {
                ...
                DataCopy(topGradLocal, gradOutputGm[batch * numQueries * numHeads * embedDims + query * numHeads * embedDims + head * embedDims],
                         embedDimsAlign);
                for (uint32_t level = 0; level < numLevels; level++) {
                    levelStartId = offsetLocal.GetValue(level);
                    h = shapesLocal.GetValue(level * 2);
                    w = shapesLocal.GetValue(level * 2 + 1);
                    offsetValue = batch * numHeads * numKeys * embedDims + head * numKeys * embedDims + levelStartId * embedDims;
