 ...
 */
#ifndef _DYNAMIC_SCATTER_GRAD_MEAN_H_
#define _DYNAMIC_SCATTER_GRAD_MEAN_H_
...
template<typename T>
class DynamicScatterGradMean : public DynamicScatterGradBase<T> {
public:
    __aicore__ inline DynamicScatterGradMean() {}
        ...
        GM_ADDR grad_point_feats, DynamicScatterGradTilingData* tilingData, TPipe* in_pipe)
    {
        ...
        this->pipe->InitBuffer(pointNumDupBuf, this->featDimAligned * sizeof(T));
...
private:
    TBuf<TPosition::VECCALC> pointNumDupBuf, pointGradBuf;
};
} // namespace DynamicScatterGrad
#endif // _DYNAMIC_SCATTER_GRAD_MEAN_H_