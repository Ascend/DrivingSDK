...
class TestHypot(TestCase):
    def test_npu_hypot_one_dim(self, device="npu"):
        ...
        npu_result = comm.npu_hypot(x.npu(), y.npu()).cpu()
        self.assertRtolEqual(npu_result.numpy(), z)

    def test_npu_hypot_one_dim_broadcast(self, device="npu"):
        x = np.random.uniform(3, 3, [1]).astype(np.float32)
        x = torch.from_numpy(x)
    ...
    def test_npu_hypot_three_dim(self, device="npu"):
    ...
    def test_npu_hypot_random_three_dim(self, device="npu"):
    ...
    def test_npu_hypot_random_three_dim_broadcast_x(self, device="npu"):
    ...
    def test_npu_hypot_random_three_dim_broadcast_y(self, device="npu"):
        x = np.random.uniform(1, 3, [35, 50, 80]).astype(np.float32)
        x = torch.from_numpy(x)
    ...
    def test_npu_hypot_large_random_dim_broadcast(self, device="npu"):
