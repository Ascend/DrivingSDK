...
public:
    ...
    {
        ASSERT(GetBlockNum() != 0 && "block dimension can not be zero!");
...
extern "C" __global__ __aicore__ void pixel_group(GM_ADDR score, GM_ADDR mask, GM_ADDR embedding, GM_ADDR kernelLabel,
    GM_ADDR kernelContour, GM_ADDR pointVector, GM_ADDR labelUpdated, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tilingData, tiling);
    KernelPixelGroup op;
    op.Init(score, mask, embedding, kernelLabel, kernelContour, pointVector, labelUpdated, workspace, &tilingData);
    op.Process();
}
