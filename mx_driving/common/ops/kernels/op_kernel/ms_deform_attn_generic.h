    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t query = startOffset; query < endOffset; query++) {
            ...
            for (uint32_t batch = 0; batch < batchSize; batch++) {
                ...
                weightOffset = (batch * numQueries + query) * numLevels * numHeads * numPoints;
                locationXOffset = weightOffset * 2;
                locationYOffset = locationXOffset + numLevels * numHeads * numPoints;
                if (inner_clean == 1) {
                ...
                for (uint32_t level = 0; level < numLevels; level++) {
                    ...
                    DataCopy(locationLocal, locationGm[locationXOffset], numPointsAlign);
                    DataCopy(locationLocal[numPointsAlign], locationGm[locationYOffset], numPointsAlign);
                    ...
                    for (uint32_t head = 0; head < numHeads; head++) {
                        ...
                        weightOffset += numPoints;
                        locationXOffset += numPoints;
                        locationYOffset += numPoints;
                        if (head < numHeads - 1) {
                            DataCopy(locationLocal, locationGm[locationXOffset], numPointsAlign);
                            DataCopy(locationLocal[numPointsAlign], locationGm[locationYOffset], numPointsAlign);
                            DataCopy(attentionWeightLocal, attentionWeightsGm[weightOffset], numPointsAlign);
        ...
        pipe->ReleaseEventID<HardEvent::MTE2_V>(eventIdMte2ToV_0);
...
private:
    ...
    DTYPE_VALUE_SPATIAL_SHAPES tmpOffset1, tmpOffset2, baseOffset, valueOffset, weightOffset, oriOffset, pointOffset,
        locationXOffset, locationYOffset, moveOffset, batchOffset, dstOffset, srcOffset, headOffset, valueLocalOffset;
