...
# pylint: disable=logging-fstring-interpolation, logging-not-lazy, too-many-lines, consider-using-dict-items, consider-using-in, unnecessary-dunder-call, duplicate-code
...
def decode_video_frames_npu(
    ...
    try:
        ...
        # Set NPU data preprocess stream
    ...
    # We need to read the entire video first to get metadata
    # reader = torchvision.io.VideoReader(video_path, "video")
    ...
    # Note: torchvision.io.read_video doesn't support random access by frame index
    # So we need to read the video and select frames
    loaded_frames = []
    loaded_ts = []

    # Read the entire video (this is inefficient but torchvision NPU backend limitation)
    # For better performance, we should use VideoReader with seek
    total_frames = video_full.shape[0] if len(video_full.shape) == 4 else 0

    for idx in frame_indices:
        if idx < total_frames:
            frame = video_full[idx]  # Shape: [C, H, W]
    ...
    # NPU frames are already in [0, 255] uint8 format
    closest_frames = closest_frames.float() / 255.0

    # Move to CPU for compatibility with the rest of the pipeline
