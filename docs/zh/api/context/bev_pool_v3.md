...
可参考模型[BEVFusion](../../../../model_examples/BEVFusion)替换bev_pool，参考模型[BEVDet](../../../../model_examples/BEVDet)替换bev_pool_v2，更具体地，对于bev_pool，可以参考调用示例简单替换；对于bev_pool_v2，应该关注bev_pool_v2调用前的计算部分，一般叫`voxel_pooling_prepare_v2`函数，保留`ranks_feat`，`ranks_depth`，`ranks_bev`计算部分，去除之后的排序，`interval_lengths`和`interval_starts`的相关计算。
