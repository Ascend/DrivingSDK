...
该报错是由于操作系统的GCC版本过高，推荐使用[DrivingSDK仓库README文档](https://gitcode.com/Ascend/DrivingSDK/blob/branch_v7.2.RC1/README.md)中的建议版本`gcc 10.2`进行编译，或使用[CenterPoint模型README文档](https://gitcode.com/Ascend/DrivingSDK/blob/branch_v7.2.RC1/model_examples/CenterPoint/README.md)中的建议版本`gcc 7.5`进行编译。
...
```
进入https://ftp.swin.edu.au/gnu/gmp/并下载gmp-6.1.0.tar.bz2
...
<a id="3-13"></a>
#### 3.13 `PivotNet`模型安装模型环境时，需安装`loguru`组件，报错：`error: subpross-exited-with-error pip subprocee to install build dependencies did not run successfully.`
